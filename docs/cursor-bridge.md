...
主 `docker-compose.yml` 也会把 CPA 挂在同名网 `cpa-cursor-bridge` 上，且这张网在两份 compose 里都是 `external: true`。脚本会先创建网络，然后再 `up`。这样 `update` / auto-update / `--force-recreate` 不会删掉还挂着 `cursor-bridge` 的网。只跑 `docker restart` 不够套用这份 compose 变更，需要一次 `bash deploy.sh start` 或 `docker compose up -d`。
